...
const PLANS = [
  {
    id: 'basico', label: 'Básico', name: 'Presencia Constante', featured: false,
    monthly: 120, setup: 300,
    desc: 'Tu negocio publicando de forma regular en tus redes principales, sin que tengas que pensar qué subir ni cuándo. La IA se encarga.',
    ideal: 'Negocios que tienen sus redes abandonadas y quieren actividad constante sin dedicarle tiempo.',
    features: [
      { text: '1 o 2 redes a elegir — Instagram, Facebook, LinkedIn o X. Donde esté tu público.', hi: false, inh: false },
      { text: 'Publicación automática regular — La IA elige el tema, redacta el texto y genera la imagen. Tú no escribes nada.', hi: true, inh: false },
      { text: 'Contenido adaptado a tu negocio — Cada publicación habla de tu sector y tu marca, no es relleno genérico.', hi: false, inh: false },
      { text: 'Tono y estilo definidos contigo — Configuramos la voz de tu marca una vez y el sistema la respeta en cada post.', hi: false, inh: false },
    ],
  },
  {
    id: 'estandar', label: 'Estándar', name: 'Marca Omnipresente', featured: true,
    monthly: 200, setup: 500,
    desc: 'Todo lo anterior en todas tus redes y con publicación diaria. Lo que un community manager te cobraría por unos pocos posts a la semana, en piloto automático.',
    ideal: 'Negocios que quieren estar en todas partes, todos los días, sin pagar una agencia de redes.',
    features: [
      { text: 'Todo lo del plan Básico', hi: false, inh: true },
      { text: 'Todas tus redes sociales — Instagram, Facebook, LinkedIn, X y TikTok, cada una con su formato y su frecuencia ideal.', hi: true, inh: false },
      { text: 'Publicación diaria — Un community manager publica 3 veces por semana y cobra 300-700€. La IA te da contenido diario por menos, sin vacaciones ni bajas.', hi: true, inh: false },
      { text: 'Imágenes generadas con IA — Cada post con su imagen original adaptada al mensaje, sin bancos de fotos repetidos.', hi: false, inh: false },
      { text: 'Informe mensual de alcance — Qué se publicó, cómo rindió y qué temas funcionan mejor. En tu correo cada mes.', hi: false, inh: false },
    ],
  },
  {
    id: 'premium', label: 'Premium', name: 'Autoridad Total', featured: false,
    monthly: 320, setup: 700,
    desc: 'Redes, blog y newsletter trabajando juntos. No solo apareces a diario: te posicionas en Google y mantienes contacto directo con tus clientes.',
    ideal: 'Negocios que quieren construir autoridad real y un canal propio que no dependa de los algoritmos.',
    features: [
      { text: 'Todo lo del plan Estándar', hi: false, inh: true },
      { text: 'Blog con artículos SEO — La IA redacta y publica artículos que te posicionan en Google. Atraes visitas sin pagar por cada clic, mes tras mes.', hi: true, inh: false },
      { text: 'Newsletter automática — Boletín periódico a tus clientes con tu contenido. Un canal directo que es tuyo, no de ninguna red social.', hi: true, inh: false },
      { text: 'Calendario editorial coherente — Redes, blog y newsletter alineados bajo una misma estrategia de temas, no piezas sueltas.', hi: false, inh: false },
      { text: 'Soporte prioritario + revisión mensual — Respuesta en menos de 4h y sesión mensual para ajustar la estrategia de contenido.', hi: false, inh: false },
    ],
  },
]

const POPUP_CFG = {
  basico:   { title: '¿Te interesa el plan Básico?',   desc: 'Publicación regular en 1-2 redes sin que toques nada. Te contamos cómo quedaría.' },
  estandar: { title: '¿Te interesa el plan Estándar?', desc: 'El más elegido. Publicación diaria en todas tus redes con imágenes IA. Valoración gratuita.' },
  premium:  { title: '¿Te interesa el plan Premium?',  desc: 'Redes + blog SEO + newsletter. Autoridad total sin tocar nada.' },
  general:  { title: '¿Hablamos sobre tu contenido?',  desc: 'Valoración gratuita de 15 minutos. Sin compromiso.' },
}
...
function ContactPopup({ plan, onClose }) {
  ...
  async function handleSubmit() {
    ...
    const payload = { nombre: name, apellidos, email, negocio: biz, sector: 'Redes sociales', plan, source: 'web-sector-rrss' }
    try { await fetch(WEBHOOK, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }) } catch {
      const sub = encodeURIComponent(`Lead web — RRSS — ${name} ${apellidos}`)
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 420, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.2)' }}>
        ...
        {!sent ? (
          <div style={{ padding: '20px 24px 24px' }}>
            {[
              { label: 'Tu nombre', val: name, set: setName, ph: 'Ej: Sara', type: 'text' },
              { label: 'Tus apellidos', val: apellidos, set: setApellidos, ph: 'Ej: Gómez Díaz', type: 'text' },
              { label: 'Tu email', val: email, set: setEmail, ph: 'tu@negocio.com', type: 'email' },
              { label: 'Tu negocio o sector (opcional)', val: biz, set: setBiz, ph: 'Nombre o sector...', type: 'text' },
            ].map(fi => (
...
export default function RrssPage({ onBack }) {
      ...
      <div style={{ minHeight: '100vh', background: C.cream, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ maxWidth: 780, margin: '0 auto', padding: '56px 24px 0', textAlign: 'center' }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', color: C.gold, marginBottom: 20 }}>
            Contenido con IA para redes, blog y newsletter
          </div>
          <h1 style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: 'clamp(26px, 4.5vw, 40px)', fontWeight: 700, color: C.navy, margin: '0 0 18px', lineHeight: 1.2 }}>
            Publica todos los días sin escribir ni una palabra
          </h1>
          <p style={{ fontSize: 16, lineHeight: 1.65, color: '#6B6560', maxWidth: 560, margin: '0 auto 12px' }}>
            La IA elige los temas, redacta, genera las imágenes y publica en tu nombre en redes, blog y newsletter. Presencia constante y profesional, en piloto automático.
          </p>
          <p style={{ fontSize: 14, color: C.muted, fontStyle: 'italic', maxWidth: 500, margin: '0 auto 28px' }}>
            El alcance orgánico premia la constancia. Un perfil que publica a diario crece; uno que publica cuando se acuerda, no.
          </p>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(26,26,46,.05)', padding: '10px 22px', borderRadius: 24, fontSize: 13, color: C.text, marginBottom: 44 }}>
            ...
            Tus redes activas desde la primera semana, sin que toques nada
          </div>
        </div>
        ...
        <div style={{ maxWidth: 820, margin: '56px auto 0', padding: '0 24px', display: 'flex', gap: 20, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[
            { Icon: Shield, title: 'Cero tiempo tuyo', text: 'No tienes que pensar qué subir, ni escribir, ni diseñar, ni acordarte de publicar. El sistema funciona solo y tú revisas resultados cuando quieras.' },
            { Icon: Calendar, title: 'El precio de una agencia, sin la agencia', text: 'Gestionar redes profesionalmente cuesta entre 300 y 700€ al mes con un community manager. Con IA el contenido es diario y el coste, menor.' },
          ].map(({ Icon, title, text }) => (
        ...
        <div style={{ maxWidth: 560, margin: '48px auto 0', padding: '0 24px 72px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: '#6B6560', lineHeight: 1.6, margin: '0 0 24px' }}>En 15 minutos vemos dónde está tu público y te decimos qué plan de contenido tiene más sentido para tu negocio.</p>
