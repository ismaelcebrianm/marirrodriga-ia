...
function ContactPopup({ plan, onClose }) {
  ...
  async function handleSubmit() {
    ...
    const payload = { nombre: name, apellidos, email, negocio: biz, sector: 'Presencia web', plan, source: 'web-sector-web' }
    try { await fetch(WEBHOOK, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }) } catch {
      const sub = encodeURIComponent(`Lead web — Web — ${name} ${apellidos}`)
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 420, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.2)' }}>
        ...
        {!sent ? (
          <div style={{ padding: '20px 24px 24px' }}>
            {[
              { label: 'Tu nombre', val: name, set: setName, ph: 'Ej: Luis', type: 'text' },
              { label: 'Tus apellidos', val: apellidos, set: setApellidos, ph: 'Ej: Martínez García', type: 'text' },
